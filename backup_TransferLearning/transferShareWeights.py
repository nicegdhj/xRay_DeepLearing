...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(0, num_epochs):
        ...
        for inputs_1, labels_1, inputs_2, labels_2 in dataloaders['train']:
        ...
        scheduler.step()
        epoch_loss_1 = running_loss_1 / len(dataloaders['train'].dataset)
        epoch_acc_normal_1 = running_corrects_normal_1.double() / len(dataloaders['train'].dataset)  # 修改mean_acc
        epoch_acc_1 = recall_score(y_true_1, y_pred_1, average='macro')

        epoch_loss_2 = running_loss_2 / len(dataloaders['train'].dataset)
        epoch_acc_normal_2 = running_corrects_normal_2.double() / len(dataloaders['train'].dataset)  # 修改mean_acc
        ...
        for inputs_1, labels_1, inputs_2, labels_2 in dataloaders['val']:
            inputs_1 = inputs_1.to(device)
            labels_1 = labels_1.to(device)
        ...
        epoch_loss_1 = running_loss_1 / len(dataloaders['val'].dataset)
        epoch_acc_normal_1 = running_corrects_normal_1.double() / len(dataloaders['val'].dataset)  # 修改mean_acc
        epoch_acc_1 = recall_score(y_true_1, y_pred_1, average='macro')

        epoch_loss_2 = running_loss_2 / len(dataloaders['val'].dataset)
        epoch_acc_normal_2 = running_corrects_normal_2.double() / len(dataloaders['val'].dataset)  # 修改mean_acc
        epoch_acc_2 = recall_score(y_true_2, y_pred_2, average='macro')
...
if __name__ == '__main__':
    ...
    optimizer_ft = optim.Adam(params_to_update, lr=flags.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-5)
    ...
    # Train and evaluate
    val_acc, val_loss, train_acc, train_loss = train_model(model_ft, dataloaders_dict, criterion,
                                                           optimizer_ft,
                                                           scheduler,
